                ...
                {packages.map((pkg) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <h3 className="min-h-6 pr-20 text-[16px] font-semibold">{packageName(pkg, locale)}</h3>
                      ...
                      <p className="package-description mt-2 min-h-10 line-clamp-2 text-[13px] leading-5 text-muted-foreground">{packageDescription(pkg, locale)}</p>
                  ...
                  <Select value={selectedPackageId} onValueChange={(value) => value && setSelectedPackageId(value)}>
                    <SelectTrigger id="package" className="request-field request-modal-field w-full">
                      ...
                      */}
                      <span>
                        {selectedPackage
                          ? `${packageName(selectedPackage, locale)} — ${formatMajorCurrency(selectedPackage.price, selectedPackage.currency)}`
                          : ""}
                      </span>
                    </SelectTrigger>
                    <SelectContent
                    ...
                    >
                      {standardPackages.map((pkg) => (
                        <SelectItem key={pkg.id} value={pkg.id}>
                          {packageName(pkg, locale)} — {formatMajorCurrency(pkg.price, pkg.currency)}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
