...
Last updated: 2026-08-11 by Agent A — status write endpoint wording corrected
...
- Each changed project status atomically creates a system note using the existing wording pattern, for example `Project status changed from Design to Development.` Same-status requests return the current project without creating a note.
...
- Project audit notes now read `Project status changed from Design to Development.` to match the existing seeded notes and notes feed.
