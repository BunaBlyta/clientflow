...
Last updated: 2026-08-27 by Claude Code — notes composer send button resized to match the text field's 52px resting height (was a 46px square, read as undersized next to the field); earlier: invoices tab KPI card alignment + serif figure fix (see below); notes composer padding fix; composer auto-compact fix + copy message (folding in Codex's concurrent notification/translation work below)
