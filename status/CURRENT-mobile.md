...
Last updated: 2026-08-27 by Claude Code — Account profile card redesigned so it isn't near-empty (see below); notes composer send button resized to match the text field's 52px resting height; earlier: invoices tab KPI card alignment + serif figure fix (see below); notes composer padding fix; composer auto-compact fix + copy message (folding in Codex's concurrent notification/translation work below)
...
- **Blocked on API lane:** the PM wants clients to edit their info (name/company/phone — not email) and password. Needs `GET /api/auth/me` extended with `companyName`/`phone`, a new `PATCH /api/auth/me`, and a new `POST /api/auth/change-password`. Full proposal in the log entry above. Mobile edit/change-password screens come after those land.
