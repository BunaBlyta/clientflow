...
Last updated: 2026-08-27 by Claude Code — invoices tab KPI card alignment fix (see below); earlier: notes composer padding fix; composer auto-compact fix + copy message (folding in Codex's concurrent notification/translation work below)
...
- Invoices tab: the two KPI cards (Outstanding / Paid to date) now use the same inner padding. The "Outstanding" card had `padding={12}` while "Paid to date" fell through to the Card default of 20, so their labels/values started at different left and top offsets and looked misaligned side by side. Both are 12 now.
