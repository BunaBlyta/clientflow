...
Last updated: 2026-08-18 10:28 by Codex — place request warnings above fields

## Current state

- Explicitly removed the accidental dark gradient strip from the light-mode request-modal title and modal surface. Light-mode modal fields now use a clearly visible light blue-gray fill with no outer border or focus shadow; required-field asterisks and inline empty-field labels use a deeper teal-blue with an alert icon above the field; dark mode is unchanged.
