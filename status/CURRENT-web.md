...
Last updated: 2026-08-18 10:29 by Codex — keep request warnings layout-stable

## Current state

- Explicitly removed the accidental dark gradient strip from the light-mode request-modal title and modal surface. Light-mode modal fields now use a clearly visible light blue-gray fill with no outer border or focus shadow; required-field asterisks and inline empty-field labels use a deeper teal-blue with an alert icon in the label row above the field, so validation does not move the textbox; dark mode is unchanged.
