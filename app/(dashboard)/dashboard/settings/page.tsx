...
function TeamSection() {
  ...
  async function handleResend(staffMember: StaffMember) {
    ...
    try {
      ...
      if (!result.emailSent) {
        throw new Error("The invitation email could not be sent. Try again.");
      }
  ...
  if (error) {
    return (
      <div className="flex min-h-40 flex-col items-center justify-center border border-status-danger/30 px-6 text-center">
        <p className="text-[13px] font-medium text-status-danger">Team couldn&apos;t load</p>
        <p className="mt-1 max-w-sm text-[12px] text-muted-foreground">{error}</p>
        <Button className="mt-4" variant="outline" size="sm" onClick={() => void loadTeam()}>
          <RefreshCw />
          Try again
        </Button>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="rounded-lg border border-border">
        {staff.map((staffMember) => (
          <div key={staffMember.id} className="flex items-center gap-2.5 border-b border-border p-4 last:border-0">
            <Avatar className="size-7">
              <AvatarFallback className="text-[11px]">{initials(staffMember.name)}</AvatarFallback>
            </Avatar>
            <div className="min-w-0">
              ...
              {resendError?.staffId === staffMember.id && (
              ...
              {!staffMember.isActive && (
                <>
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    disabled={resendingStaffId !== null}
                    onClick={() => void handleResend(staffMember)}
                  >
        ...
        {staff.length === 0 && (
          <p className="px-4 py-10 text-center text-[13px] text-muted-foreground">No team members yet.</p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <form onSubmit={handleInvite} className="flex max-w-xl flex-wrap items-end gap-2">
          ...
          <Button type="submit" disabled={isInviting}>
            {isInviting ? <LoaderCircle className="animate-spin" /> : <Mail />}
