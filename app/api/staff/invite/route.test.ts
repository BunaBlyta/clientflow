...
describe('POST /api/staff/invite', () => {
  ...
  it('is mounted at the browser path used by Settings', () => {
    const routePath = fileURLToPath(new URL('./route.ts', import.meta.url));

    expect(routePath).toMatch(/[\\/]app[\\/]api[\\/]staff[\\/]invite[\\/]route\.ts$/);
  ...
  it('returns 401 for an unauthenticated request', async () => {
    ...
    const response = await POST(request({ email: 'jordan@example.com', name: 'Jordan Ellis' }));
