...
```
Read AGENTS.md and the CURRENT STATE block at the top of STATUS.md before
anything else. They are the source of truth — trust them over anything you
remember or infer from the code.
...
- Update the CURRENT STATE block in STATUS.md before you stop, and append a
  log entry below it. Plain language a non-engineer could follow.
...
```
Read AGENTS.md and the CURRENT STATE block at the top of STATUS.md first.
...
Commit at the end of every task and update STATUS.md before you stop.
```
...
skimmed it, missed the current state, and re-derived it badly. Hence the CURRENT
STATE block at the top — short enough to actually be read, and overwritten each
session rather than appended to.
