...
- **RULE 2 — Before you stop, overwrite your own lane's state file and add a log entry.** Your lane's file is `status/CURRENT-backend.md`, `status/CURRENT-web.md` or `status/CURRENT-mobile.md`. **Write only your own — never another lane's, and never `STATUS.md`, which Buna owns.** Then create a *new* file at `status/log/YYYY-MM-DD-HHMM-agent-task.md`; never edit an existing log file. Every file has exactly one writer, so collisions are structurally impossible — do not break that by editing someone else's. If your state file is stale, the next session starts blind and re-derives the state badly, which is exactly how the 10 August mess happened.
...
- **CURRENT SETUP, from 2026-08-11 midday: three Codex agents, split by FEATURE, not by layer.** Each owns a complete vertical — API routes, web pages and mobile screens for its own feature. Find your vertical below and read only your own `status/CURRENT-*.md`.

  | Agent | Vertical | Owns |
  |---|---|---|
  | **A** | Auth | `app/api/auth/**`, `app/api/_lib/auth.ts`, `app/(auth)/**`, `middleware.ts`, `mobile/app/(auth)/**`, `mobile/store/auth-store.ts`, `status/CURRENT-auth.md` |
  | **B** | Stripe & invoices | `app/api/stripe/**`, `app/api/invoices/**`, `prisma/invoice-state.ts`, `components/dashboard/invoice-*`, `components/dashboard/create-invoice-dialog.tsx`, `app/(dashboard)/dashboard/invoices/**`, `mobile/app/(app)/projects/[id]/invoices/**`, `status/CURRENT-stripe.md` |
  | **C** | Data wiring | `app/api/projects/**`, `app/api/clients/**`, `app/api/notes/**`, `app/api/notifications/**`, `app/api/requests/**`, `app/(dashboard)/dashboard/{clients,projects,analytics,notifications,settings}/**`, `lib/store.ts`, `lib/mock-data.ts`, `lib/analytics.ts`, `mobile/app/(app)/projects/index.tsx`, `mobile/app/(app)/notes*`, `mobile/app/(app)/notifications.tsx`, `status/CURRENT-wiring.md` |

- **SHARED FILES — do not edit without telling Buna first.** Because the split is by feature, these are the places three agents will collide. Read them freely; changing them needs a heads-up:
  - `lib/types.ts` and `mobile/lib/types.ts` — agent C is the tiebreaker owner. A and B propose changes through Buna.
  - `prisma/schema.prisma` and `prisma/migrations/` — **only one agent may hold a migration at a time.** Two concurrent migrations will conflict and can corrupt migration history. Ask Buna before running `prisma migrate`.
  - `package.json` / `package-lock.json` — **only one agent may run `npm install` at a time.** Concurrent installs corrupt the lockfile. Ask first.
  - `app/api/_lib/prisma.ts`, `components/ui/**`, `app/globals.css`, `app/layout.tsx` — read-only unless agreed.
- **`npm run verify` will sometimes fail because of ANOTHER agent's half-written file.** That is expected with three agents in one checkout. Re-run it. If it still fails, check whether the error is in a file you own — if it is not yours, do not fix it, report it.
- **Never `git add -A`.** Commit your own paths explicitly, including your own `status/CURRENT-*.md`. Another agent almost certainly has work in progress.
- **Ownership split (historical, from the earlier layer-based setup):** Codex CLI owns the backend — the Prisma schema (defined first, once, sequentially, before anything else fans out, since every other lane depends on it), API routes, the Stripe integration and webhook handler, auth logic, and the database seed script (mock clients/projects/invoices across a spread of statuses, so the dashboard and analytics have real data to show from the first login — an empty dashboard doesn't demonstrate anything). Claude Code owns all frontend on both platforms — the web landing/marketing page, the web staff dashboard, and the mobile app — and can spawn subagents to run these as parallel lanes, since they're separate codebases/route trees with no file overlap. Once the schema is locked, further fan-out is fine on both sides (e.g. separate backend lanes for Stripe/invoices vs. requests/notifications vs. analytics aggregation) as long as lanes don't touch the same files.
