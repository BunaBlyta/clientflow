...
- **Isolation:** work in your own git worktree/branch — Codex CLI touching only `prisma/` and the API route directories, Claude Code touching only frontend directories (web pages/components, and the separate Expo app folder) and never the schema or API route files. Never work directly on `main`.
...
- **Before ending a session:** update `STATUS.md` with what changed, what's left, and any blockers or assumptions made.
...
- **Definition of done for any task:** code runs, tests pass, no linter errors, `STATUS.md` updated in plain language, and the change matches what's described in `/docs/SPEC.md` — not more, not less.
